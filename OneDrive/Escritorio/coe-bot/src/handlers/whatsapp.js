 ...
 * Incluye: flujo completo de intakes, horario de atención, mensaje fuera de horario.
 */
...
const {
  ...
  buildTransferMessage, buildAreaNotification,
  INTAKE_STEPS,
...
async function routeMessage(from, interactionId, textBody) {
  ...
  if (session.stage === "intake_cirugia") {
    const nombre = textBody?.trim();
    updateSession(from, { stage: "transferred" });
    await sendAll(from, buildIntakeCirugiaConfirmation(nombre, session.areaId));
    await notifyArea("cirugia", from, `Solicitud de cirugía — Paciente: ${nombre}`);
    return;
  }

  // ── Verificar horario ANTES de procesar cualquier otra interacción ────
  // Excepción: si ya está en un flujo activo, dejamos que lo termine
  if (!estaAbierto() && session.stage === "idle") {
    const nombre = session.intakeCita?.nombre || null;
    await sendMsg(from, {
      type: "text",
      text: { body: mensajeFueraDeHorario(nombre) },
    });
    // Igual mostramos el menú para que quede registrada su intención
    await delay(500);
  ...
  if (interactionId === "consultas" || interactionId === "start_intake_cita") {
    updateSession(from, {
      stage: "intake_cita", areaId: "consultas",
      intakeCita: {}, intakeStep: INTAKE_STEPS[0].key,
    });
    await sendAll(from, [
  ...
  if (interactionId?.startsWith("menu_")) {
    await sendAll(from, buildFaqMenu(interactionId.replace("menu_", "")));
    return;
  }

  // ── Texto libre / primer mensaje → bienvenida ─────────────────────────
  // Si está fuera de horario y es el primer contacto, avisar primero
  if (!estaAbierto()) {
    await sendMsg(from, {
      type: "text",
      text: { body: mensajeFueraDeHorario(null) },
    });
    await delay(600);
  }
  ...
  for (const message of messages) {
    ...
    if (message.type === "interactive") {
      ...
      if (ia.type === "button_reply") {
        ...
        textBody      = ia.button_reply.title;
      } else if (ia.type === "list_reply") {
        interactionId = ia.list_reply.id;
        textBody      = ia.list_reply.title;
      }
